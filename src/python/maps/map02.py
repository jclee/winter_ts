...
def to1():
    ...
    yield from system.engineObj.mapSwitchTask('map01.ika-map', (1 * 16, y))

def to3():
    offset_from = 14 * 16  # first vertical pos possible
    offset_to = 6 * 16  # first vertical pos possible
    y = system.engineObj.player.y - offset_from + offset_to
    yield from system.engineObj.mapSwitchTask('map03.ika-map', (98 * 16, y))
...
def to6():
    ...
    yield from system.engineObj.mapSwitchTask('map06.ika-map', (x, 38 * 16))

def to43():
    yield from system.engineObj.mapSwitchTask('map43.ika-map', (1 * 16, system.engineObj.player.y))
